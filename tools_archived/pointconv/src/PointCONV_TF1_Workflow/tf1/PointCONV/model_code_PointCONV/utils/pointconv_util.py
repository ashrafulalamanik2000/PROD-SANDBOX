...
from model_code_PointCONV.utils.tf_ops.sampling.tf_sampling import gather_point
from model_code_PointCONV.utils.tf_ops.sampling.tf_sampling import farthest_point_sample
from model_code_PointCONV.utils.tf_ops.grouping.tf_grouping import query_ball_point
from model_code_PointCONV.utils.tf_ops.grouping.tf_grouping import group_point
...
def kernel_density_estimation_ball(pts, radius, sigma, N_points = 128, is_norm = False):
    with tf.variable_scope("ComputeDensity") as sc:
        ...
        scale = tf.math.divide(1.0, tf.expand_dims(tf.cast(pts_cnt, dtype = tf.float32), axis = -1))
...
def kernel_density_estimation(pts, sigma, kpoint = 32, is_norm = False):
    with tf.variable_scope("ComputeDensity") as sc:
        ...
        R = tf.sqrt(sigma)
        xRinv = tf.math.divide(grouped_pts, R)
        quadform = tf.reduce_sum(tf.square(xRinv), axis = -1)
        logsqrtdetSigma = tf.log(R) * 3
        mvnpdf = tf.exp(-0.5 * quadform - logsqrtdetSigma - 3 * tf.log(2 * 3.1415926) / 2)
        mvnpdf = tf.reduce_sum(mvnpdf, axis = 2, keepdims = True)
