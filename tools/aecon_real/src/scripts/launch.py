"""Host-side AECON preflight, ECR login, image pull, and Docker launcher."""
...
def main() -> int:
    ...
    stages = _option(pipeline_args, "--stages", "yaml,organize,metadata,pano,colorize")
    report = validate(root, stages)
    if known.json:
    ...
    command = [
        ...
        "-v", f"{scripts}:/app", "-v", f"{root}:/data",
        "-e", "AECON_ENGINE=python3 /app/pano_generator.py",
        "-e", "AECON_PANO_WORKERS=16",
    ]
    ...
    defaults = [
        ...
        "--project-workers", "1", "--las-workers", "4", "--threads", "8",
    ]
    command += [IMAGE, "/root/miniconda3/envs/pdal/bin/python", "/app/aecon.py", "/data", *defaults, *pipeline_args]
